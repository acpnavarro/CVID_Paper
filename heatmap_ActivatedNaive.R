...
#Read DEG with padjust < 0.05
AN <- read_excel("data/DEG_AN_padj0.05.xlsx")
AN <- as.data.frame(AN)
...
s_mat2<-s_mat[, grep("^AN.CVID|^AN.HD", names(s_mat))]
...
df_ann=ann_sample %>% 
  filter(str_detect(Sample, "^AN.CVID|^AN.HD"))
...
pheatmap(s_mat2,
         ...
         fontsize_col = 8,
         border_color = NA,
         main = "Activated Naïve B Cells: CVID vs. HD")
