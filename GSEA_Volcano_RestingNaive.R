...
df <-read_excel("data/df.IN_CVIDxHD.xlsx")
# log2 fold change 
...
nes_value <- round(gse@result[gse@result$ID == "GO:0002757", "NES"], 2)
...
# Extract NES and FDR (adjusted p-value) for STAT5A_04
stat5_data <- data_em2 %>% filter(ID == "STAT5A_04")
NES_stat5 <- round(stat5_data$NES, 2)
FDR_stat5 <- signif(stat5_data$p.adjust, 2)
...
ggplot(data = df, aes(x = log2FoldChange, y = -log10(pvalue))) +
  ...
  coord_cartesian(ylim = c(0, 100), xlim = c(-5, 15)) +
...
ggsave("volcanoIN_CVIDxHD.png", dpi = 300, width = 10, height = 8)
