...
#Read
IB <- read_excel("data/DEG_IB_padj0.05.xlsx")
...
s_mat2<-s_mat[, grep("^IB.CVID|^IB.HD", names(s_mat))]
...
df_ann=ann_sample %>% 
  filter(str_detect(Sample, "^IB.CVID|^IB.HD"))
df_ann <- data.frame(df_ann, row.names = 1)
df_ann$Group<-gsub("*_Nonactivated_BrightMBCs", "", df_ann$Group)
...
pheatmap(s_mat2,
         name = "Z-score",  # 👈 prettier name here
         color = my_palette,
         scale = "none",
