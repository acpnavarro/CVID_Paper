...
#Read
AB <- read_excel("data/DEG_AB_padj0.05.xlsx")
...
s_mat2<-s_mat[, grep("^AB.CVID|^AB.HD", names(s_mat))]
...
df_ann=ann_sample %>% 
  filter(str_detect(Sample, "^AB.CVID|^AB.HD"))
df_ann <- data.frame(df_ann, row.names = 1)
df_ann$Group<-gsub("*_Activated_BrightMBCs", "", df_ann$Group)
...
pheatmap(s_mat2,
         ...
         fontsize_col = 8,
         border_color = NA,
         main = "Activated Memory B Cells: CVID vs. HD")
