...
# Unique genes list
unique_genes <- read_excel("data/unique_genes_CVID_Naive.xlsx")
...
write.xlsx(as.data.frame(ego), file = "ORA_GO_CVID_N.xlsx", rowNames = FALSE)
write.xlsx(as.data.frame(kegg_ora), file = "ORA_KEGG_CVID_N.xlsx", rowNames = FALSE)
write.xlsx(as.data.frame(reactome_ora), file = "ORA_Reactome_CVID_N.xlsx", rowNames = FALSE)
write.xlsx(as.data.frame(hallmark_ora), file = "ORA_Hallmark_CVID_N.xlsx", rowNames = FALSE)

# Create individual plots
go_barplot <- barplot(ego, showCategory = 10, title = "GO Biological Process CVID_Naïve")
reactome_dotplot <- dotplot(reactome_ora, showCategory = 10, title = "Reactome Enrichment CVID_Naïve")
...
# Show combined plot
print(combined_plot)
ggsave("combined_ORA_GO_Reactome_CVID_Naive.png", plot = combined_plot, width = 14, height = 6, dpi = 300)
